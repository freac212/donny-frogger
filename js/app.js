...
*/
// HTML constants
var levelsHTML = document.getElementById("levels");
var livesHTML = document.getElementById("lives");
var highscoreHTML = document.getElementById("highscore");
var gamemodeButton = document.getElementById("gamemode-button"); // Important constant positions
//                                     X                Y

var waterArrayOfPostions = [[0, 101, 202, 303, 404], [-18]];
var arrayOfPositionsX = [0, 101, 202, 303, 404];
var arrayOfPositionsY = [-18, 65, 148, 231, 314, 397];
var stoneArrayOfPostions = [65, 148, 231]; // Disable the arrow keys moving the window scroll bar
...
}, false); // Game Constructor: It has effectively everything that runs the levels, start, lives, and game over. Also endless and debug mode.
...
function () {
  function Game() {
    ...
    if (this.debugMode) {
      ...
      console.log("Level: ".concat(this.level + 1));
  ...
  _createClass(Game, [{
    key: "gameOver",
    value: function gameOver(gameOverCondition) {
      ...
      if (gameOverCondition === "win") {
        ...
        if (this.debugMode) {
          ...
          console.log("Changing gamemodes to: ".concat(tempGamemode));
        }
      } else {
      ...
      this.level = 0; // Reset lives to default

      this.lives = this.livesDefault; // Reset player pos

      this.resetPlayerPos(); // Reset bug speed

      this.bugSpeedMax = this.bugSpeedMaxDefault; // Reset bug pos -> change level to 1 (0)

      this.resetBugPos(); // Reset HTML
    ...
    value: function reset() {
      ...
      if (this.lives <= 0) {
        ...
        if (this.debugMode) {
          console.log("Lives remaining: ".concat(this.lives));
        }
    ...
    value: function levelComplete() {
      ...
      if (this.level === this.maxLevels) {
        ...
        this.resetPlayerPos();
        this.resetBugPos(); // Update Bug Speed
      ...
      if (this.debugMode) {
        ...
        for (i = 0; i < allEnemies.length; i++) {
          console.log("Bug ".concat(i, "'s position: ").concat(allEnemies[i].y));
          console.log("Bug ".concat(i, "'s speed: ").concat(allEnemies[i].speed));
        }
      }
    }
  }, {
    key: "resetPlayerPos",
    value: function resetPlayerPos() {
      // Reset player pos
    ...
    value: function resetBugPos() {
      ...
      allEnemies.forEach(function (element) {
        ...
        element.speed = randomNumMinMax(_this.bugSpeedMin, _this.bugSpeedMax);
      });
    }
  }, {
    key: "updateHighScore",
    ...
    value: function updateHTML() {
      ...
      levelsHTML.innerText = "Level ".concat(this.level + 1, " of ").concat(this.maxLevels); // Loop to update the correct amount of hearts

      var tempHeart = "";

      for (var _i = 0; _i < this.lives; _i++) {
        tempHeart += "<i style=\"color: red; \"class=\"fas fa-heart\"></i>";
      }

      livesHTML.innerHTML = "Lives remaining: ".concat(tempHeart); // Update the highscore

      highscoreHTML.innerText = "Level Highscore: ".concat(this.levelHighScore); // Update the gamemode button
...
var game = new Game(); // ##### ENEMY CONSTRUCTOR #####

var Enemy = function Enemy() {
  var x = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : -101;
  var y = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 236;
  var speed = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 100;
  this.x = x;
  this.y = y;
  this.speed = speed;
  this.imageSizeX = 80;
  this.imageSizeY = 80; // The image/sprite for our enemies, this uses
...
var Player = function Player() {
  // Player sprite
  this.sprite = "images/char-boy.png"; // Player Movement Size

  this.movementSizeX = 101;
  this.movementSizeY = 83; // Player Debug movement positions
  ...
  this.charSizeX = 80;
  this.charSizeY = 80; // Starting position
...
Player.prototype.update = function () {
  ...
  allEnemies.forEach(function (element) {
    if (_this2.x + 10 >= element.x && _this2.x + 10 <= element.x + 10 + element.imageSizeX && _this2.y + 60 >= element.y + 60 && _this2.y + 60 <= element.y + 60 + element.imageSizeY) {
      if (game.debugMode) {
        console.log("dead");
      } else {
        game.reset();
    ...
    if (_this2.x + 90 >= element.x && _this2.x + 90 <= element.x + 10 + element.imageSizeX && _this2.y + 60 >= element.y + 60 && _this2.y + 60 <= element.y + 60 + element.imageSizeY) {
      if (game.debugMode) {
        console.log("dead");
      } else {
        game.reset();
    ...
    if (_this2.x + 90 >= element.x && _this2.x + 90 <= element.x + 10 + element.imageSizeX && _this2.y + 60 + 80 >= element.y + 60 && _this2.y + 60 + 80 <= element.y + 60 + element.imageSizeY) {
      if (game.debugMode) {
        console.log("dead");
      } else {
        game.reset();
    ...
    if (_this2.x + 10 >= element.x && _this2.x + 10 <= element.x + 10 + element.imageSizeX && _this2.y + 60 + 80 >= element.y + 60 && _this2.y + 60 + 80 <= element.y + 60 + element.imageSizeY) {
      if (game.debugMode) {
        console.log("dead");
      } else {
        game.reset();
...
Player.prototype.render = function () {
  ...
  if (game.debugMode) {
    ...
    ctx.beginPath();
    ctx.rect(this.x, this.y, 101, 171);
    ctx.stroke(); // Player hitbox rectangle
...
var player = new Player(); // Creating the array that will contain all the made enemies

var allEnemies = []; // This creates the enemies, placing the enemy objects into an array with their randomly set properties(position, speed)
...
function createEnemies() {
  for (var _i2 = 0; _i2 < game.numOfEnemies; _i2++) {
    var position = stoneArrayOfPostions[randomIndex(3)];
    var randomBugSpeed = randomNumMinMax(game.bugSpeedMin, game.bugSpeedMax);

    if (game.debugMode) {
      console.log("Bug ".concat(_i2, "'s position: ").concat(position));
      console.log("Bug ".concat(_i2, "'s speed: ").concat(randomBugSpeed));
    }
...
;
createEnemies(); // This listens for key presses and sends the keys to your
...
gamemodeButton.addEventListener("click", function (e) {
  if (game.endlessMode) {
